...
One line per autonomous run: date, the two segments chosen, and why.
Format: `YYYY-MM-DD | seg1: <industry> / <submarket> · seg2: <industry> / <submarket> | why: <one sentence>`
...
## Second inbox — what the playersnext connection revealed (2026-07-28)
Connecting `tbaity@playersnext.com` **swapped** the Gmail connector rather than
adding a second account; the 10 drafts created today are intact in
twbaity@alumni.unc.edu but unreadable while the connector points elsewhere.
Durable fix is Gmail's own "Check mail from other accounts" import so one
connector sees both. 201 sent threads sit in the playersnext account.

Confirmed identities: Jake Hawkins (JH) jhawkins@playersnext.com, Max Lehmann
(ML) mlehmann@playersnext.com, Bryce Jeske (BJ) bryce@playersnext.com —
signing off as Finley Assistant GM as recently as Jan 2026, so his departure is
NOT established. Also present: rob@, jglen@, pbrown@, paderhold@, jtpeebles@.

**Two campaigns share this inbox** (Tyler, 2026-07-28): Finley corporate
partnerships, and a PLAYERSNEXT youth-sports/foundation push. Only the Finley
threads may feed the tracker — see SKILL.md Step 0 for the discriminator.

Finley-side leads found but NOT yet in the tracker — capture on the next run:
- **Dwight Anderson** (Ospraie Management) dwighta@ospraie.com, assistant Dawn
  Pennisi dawn.pennisi@ospraie.com. Old Well Patron agreement **signed** (Mar
  2026). Tracker has him Signed: 100% with no contact detail and no revenue
  figure — the signed agreement and invoice are in this thread.
- **Matt Cain**, Senior Executive Director, UNC (Matt_Cain@med.unc.edu) — live
  thread on Olympic-sport tickets under Dwight's Old Well Patron benefit.
- **Caitlin "Poppy" Van Sickle**, UNC Field Hockey assistant coach
  (cvansick@email.unc.edu) — 16-player Boutique Outing held 22 April 2026,
